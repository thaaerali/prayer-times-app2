// nahj-sermons.js - معالجة الخطب فقط
class NahjSermons {
    constructor(baseURL = 'https://raw.githubusercontent.com/thaaerali/nahj-data/main/') {
        this.baseURL = baseURL;
        this.sermonsIndexURL = this.baseURL + 'sermons/sermons-index.json';
        this.currentSermon = null;
        this.sermonsIndex = [];
        this.currentSermonId = 1;
        this.totalSermons = 0;
        
        // عناصر DOM الخاصة بالخطب
        this.elements = {
            sermonsContent: null,
            sermonContainer: null,
            prevSermonBtn: null,
            nextSermonBtn: null,
            gotoInput: null,
            gotoBtn: null,
    ...
    async init(containerId = 'sermons-container') {
        console.log('📚 جاري تهيئة قسم الخطب...');
        
        await this.loadSermonsIndex();
        this.setupContainer(containerId);
        await this.loadSermon(1);
        
        return this;
    }
    
    async loadSermonsIndex() {
        try {
            console.log('📋 جاري تحميل فهرس الخطب...');
            
            const response = await fetch(this.sermonsIndexURL);
            if (!response.ok) throw new Error(`فهرس الخطب: ${response.status}`);
            
            const data = await response.json();
            this.sermonsIndex = data.sermons_index || [];
            this.totalSermons = this.sermonsIndex.length;
            
            console.log(`✅ تم تحميل فهرس الخطب: ${this.totalSermons} خطبة`);
            return this.sermonsIndex;
            
        } catch (error) {
            console.error('❌ خطأ في فهرس الخطب:', error);
            return [];
        }
    }
    ...
    async loadSermon(sermonId) {
        try {
            ...
            if (!sermonInfo) throw new Error(`الخطبة ${sermonId} غير موجودة في الفهرس`);
            
            // تحميل ملف الخطبة
            const sermonURL = this.baseURL + sermonInfo.file;
            const response = await fetch(sermonURL);
            
            if (!response.ok) throw new Error(`الخطبة ${sermonId}: ${response.status}`);
    ...
    setupContainer(containerId) {
        ...
        container.innerHTML = `
                        ...
                        <div class="col-md-3 text-end">
                            <div class="dropdown">
                                <button class="btn btn-outline-secondary btn-sm dropdown-toggle" 
                                    ...
                                    <i class="bi bi-list-ul"></i> فهرس الخطب (${this.totalSermons})
                                </button>
            ...
            <div id="sermon-content-container"></div>
        `;
    ...
    updateDOMElements() {
        ...
        this.elements.currentTitle = document.getElementById('current-sermon-title');
        this.elements.sermonCounter = document.getElementById('sermon-counter');
        this.elements.gotoInput = document.getElementById('goto-sermon-input');
        this.elements.gotoBtn = document.getElementById('goto-sermon-btn');
        this.elements.sermonsList = document.getElementById('sermons-list-menu');
        this.elements.sermonContainer = document.getElementById('sermon-content-container');
    }
    ...
    setupNavigationEvents() {
        ...
        if (this.elements.gotoBtn && this.elements.gotoInput) {
            this.elements.gotoBtn.addEventListener('click', () => {
                const sermonId = parseInt(this.elements.gotoInput.value);
                if (sermonId >= 1 && sermonId <= this.totalSermons) {
                    this.loadSermon(sermonId);
                } else {
                    alert(`الرجاء إدخال رقم بين 1 و ${this.totalSermons}`);
                }
            });
    ...
    loadSermonsList() {
        if (!this.elements.sermonsList || this.sermonsIndex.length === 0) return;
        ...
        for (const [category, sermons] of Object.entries(sermonsByCategory)) {
            listHTML += `
                <h6 class="dropdown-header">${category} <span class="badge bg-secondary">${sermons.length}</span></h6>
                ${sermons.map(sermon => `
        ...
        this.elements.sermonsList.innerHTML = listHTML;
        ...
        document.querySelectorAll('.sermon-list-item').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                const dropdown = bootstrap.Dropdown.getInstance(document.getElementById('sermons-list-btn'));
                dropdown.hide();
            });
        });
    }
    ...
    updateNavigationUI() {
        // تحديث العنوان
        if (this.elements.currentTitle && this.currentSermon) {
        ...
        if (this.elements.prevSermonBtn) {
            ...
            this.elements.prevSermonBtn.innerHTML = this.currentSermonId <= 1 ? 
                '<i class="bi bi-chevron-right"></i>' : 
        ...
        if (this.elements.nextSermonBtn) {
            ...
            this.elements.nextSermonBtn.innerHTML = this.currentSermonId >= this.totalSermons ?
                '<i class="bi bi-chevron-left"></i>' :
        ...
        if (this.elements.gotoInput) {
            this.elements.gotoInput.value = this.currentSermonId;
            this.elements.gotoInput.max = this.totalSermons;
        }
    ...
    renderCurrentSermon() {
        if (!this.currentSermon || !this.elements.sermonContainer) return;
        ...
        this.elements.sermonContainer.innerHTML = html;
    ...
    showError(message) {
        if (this.elements.sermonContainer) {
            this.elements.sermonContainer.innerHTML = `
    ...
    // دالة للمساعدة في البحث
    searchSermons(keyword) {
        return this.sermonsIndex.filter(sermon => {
            return sermon.title.includes(keyword) || 
                   sermon.subtitle?.includes(keyword) ||
                   sermon.description?.includes(keyword) ||
                   sermon.keywords?.some(kw => kw.includes(keyword));
        });
    }
}
